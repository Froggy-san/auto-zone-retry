...
export function SalesPie({ salesData }: Props) {
  // const products = salesData.map((item, i) =>
  //   item.length ? item[i].productsToSell : ""
  // );

  const flatData = salesData.flat();
  const products = flatData.map((item) => item.productsToSell).flat();
  const productIds = Array.from(new Set(products.map((pro) => pro.id)));
  ...
  console.log(productsPie, "AAAAs");

  if (productsPie.length > 6) {
    const firstSix = productsPie.slice(0, 6);
    const fromSix = productsPie.slice(6);
    console.log(firstSix, "First six");
    console.log(fromSix, "PIEE");
  }
  ...
  return (
    <div className=" flex flex-col items-center   sm:flex-row gap-3 sm:gap-10">
